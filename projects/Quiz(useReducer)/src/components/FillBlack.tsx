...
const answerTheme = {
	correct: "flex-1 h-fit px-1.5 border-2 py-1 outline-0 transition-all ease-in-out delay-200 rounded-sm animate-error-input border-green-600 shadow-sm shadow-green-600",
...
function FillBlack(props: Props) {
	const [questionText, setQuestionText] = useState<string>(props.question.question);
	...
	const handleClick = () => {
		...
		if (inputRef.current && inputRef.current?.value) {
			setQuestionText((text) => text.replace("___", inputRef.current!.value));
		}
